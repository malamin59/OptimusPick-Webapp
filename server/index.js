...
async function run() {
  try {
    ...
    const recommenderCollection = client
      .db("AllQueries")
      .collection("recommendation");
    const userExperience = client.db("AllQueries").collection("experience");
    ...
    app.post("/experience", async (req, res) => {
      ...
      const result = await userExperience.insertOne(experience);
      res.send(result);
    });

    /* get user experience data */
    app.get("/userExperience", async (req, res) => {
      const result = await userExperience
        .find()
        .sort({ create_At: -1 })
        .toArray();
      res.send(result);
    ...
    app.get("/getAllRecommendation", async (req, res) => {
      const result = await recommenderCollection
        .find()
        .sort({ createdAt: -1 })
        .toArray();
      res.send(result);
    ...
    app.get(
      "/getMyReCommendation/:recommendedUserEmail",
      verifyFirebaseToken,
      async (req, res) => {
    ...
    app.delete(
      "/deleteRecommenderData/:deleteId/:queryId",
      async (req, res) => {
